...
function ClientFinance() {
  ...
  return (
    <div className="container-fluid py-4 bg-white" style={{ minHeight: "100vh", fontSize: "14px" }}>
              ...
              {products.length === 0 ? (
                ...
                products.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>{item.image ? <img src={`http://localhost:5000/uploads/${item.image}`} alt="product" style={{ width: "40px", borderRadius: "4px" }} /> : "-"}</td>
      ...
      <AddCreditModal show={showCreditModal} handleClose={() => setShowCreditModal(false)} />
      <AddDebitModal show={showDebitModal} handleClose={() => setShowDebitModal(false)} />
